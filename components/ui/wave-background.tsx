  ...
  const movePoints = (time: number) => {
    ...
    lines.forEach((points) => {
      points.forEach((p: Point) => {
        const move = noise((p.x + time * 0.008) * 0.002, (p.y + time * 0.003) * 0.0016) * 10
        ...
        const l = Math.max(240, mouse.vs)

        if (d < l) {
          const s = 1 - d / l
          const f = Math.cos(d * 0.001) * s

          p.cursor.vx += Math.cos(mouse.a) * f * l * mouse.vs * 0.00048
          p.cursor.vy += Math.sin(mouse.a) * f * l * mouse.vs * 0.00048
        }
        ...
        p.cursor.x = Math.min(70, Math.max(-70, p.cursor.x))
        p.cursor.y = Math.min(70, Math.max(-70, p.cursor.y))
      })
    })
  }
